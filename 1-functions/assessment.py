...
def my_hometown(town_name):
    """Tests whether town name passed is my hometown.

    Returns True or False.
    """

    if town_name == "Walnut Creek":
        return True
    else:
        return False
...
def greeting(town_name, first_name, last_name):
    """Prints greeting depending on hometown.
    """

    same_hometown = my_hometown(town_name)
    full_name = first_and_last_name(first_name, last_name)

    if same_hometown == True:
...
def calculate_price(base_price, state, tax = 0.05):
    ...
    if state == "CA":
        return (base_price + base_price * tax) * 1.03
    elif state == "PA":
        return base_price + base_price * tax + 2
    elif state == "MA":
        if base_price < 100:
            return base_price + base_price * tax + 1
        else:
            return base_price + base_price * tax + 3
    else:
...
def append_args(start_list, *args):
    """Adds args to end of list.
    """

    arg_list = list(args)  #converts args to list from tuple

    return start_list + arg_list
...
def word_multiplication(word):
    """Creates list with word and word times 3.
    """

    def multiply_by_3():
        """Multiplies word by 3 and makes a string
        """
        return word * 3

    return word, multiply_by_3()
